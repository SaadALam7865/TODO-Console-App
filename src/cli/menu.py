...
def handle_add_task(app):
    ...
    try:
        ...
        due_date = input("Enter due date (YYYY-MM-DD format, optional): ").strip()
        if not due_date:
            due_date = None
        ...
        task = app.add_task(title, description, due_date, priority, tags)
...
def handle_update_task(app):
    ...
    try:
        ...
        due_date = input(f"Enter new due date (current: '{current_task.due_date}', YYYY-MM-DD format, press Enter to keep current): ").strip()
        ...
        updated_task = app.update_task(task_id, new_title, new_description, new_due_date, new_priority, new_tags)
